...
export default function PumpStatusCard({ pumpOn, manualMode, pumpRuntime }: PumpStatusCardProps) {
  const [displaySeconds, setDisplaySeconds] = useState(0);
  const lastMqttUpdate = useRef(Date.now());
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Sync from MQTT data and start local tick
  useEffect(() => {
  ...
  // Dash pattern for the spinning ring segments
      ...
      <div className="relative" style={{ width: size, height: size }}>
        ...
        >
          <circle
            ...
            style={{
              filter: isActive ? "drop-shadow(0 0 6px hsl(142 71% 45% / 0.5))" : undefined,
            }}
          />
        </svg>
